...
public:
	...
	TPos finder2(const int k, const TPos& v);
...
typename SearchTree<E>::TPos SearchTree<E>::finder(const K& k,  const TPos& v){
    ...
    if((*v).key() > k) return finder(k,v.left());
    else if((*v).key() < k) return finder(k,v.right());
    else return v;
}

template <typename E>
typename SearchTree<E>::TPos SearchTree<E>::finder2(const int k, const TPos& v) {
	if (v.isExternal()) return v;
	if ((*v).key() > k) return finder2(k, v.left());
	else if ((*v).key() < k) return finder2(k, v.right());
	else return v;
}
